...
//! let short_metadata = cut_metadata(&data.as_ref(), &mut (), &full_metadata, &specs_westend).unwrap();
...
//! metadata's `PortableRegistry` with the hash of SCALE-encoded
//! [`MetadataDescriptor`]. `MetadataDescriptor` contains other relatively short
//! data necessary for decoding and appropriate data representation.
//!
//! `MetadataDescriptor` is versioned to simplify compatibility check on the
//! hardware side.
//!
